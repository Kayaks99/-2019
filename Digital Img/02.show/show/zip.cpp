...
struct haffNode{
    ...
    int data;//这个用来存字符本身，比如字符是'c'，data='c';
    struct haffNode *leftChild=NULL,*rightChild=NULL;//定义左右子节点指针
};
char code[256][256];//用二维数组来储存字符的哈夫曼编码，其中第一维的下标表示的是这个字符的ASCII码
haffNode hleft[256];//用来储存所有的左子节点
haffNode hright[256];//用来储存所有的右子节点
...
void creatHaffman(haffNode *node,int length){
    ...
    hleft[length] = node[length-2],hright[length]=node[length-1];//定义子字节，用来存数组最后的两个节点
    parent.weight = hleft[length].weight + hright[length].weight;//父节点的权重等于两个子节点的 权重
    //储存两个子节点,因为parent.leftChild是指针类型，所以赋值的时候要加&
    parent.leftChild= &hleft[length];
    parent.rightChild = &hright[length];
...
void coding(haffNode *node,char *keepCode,int length){
    ...
    if(node->leftChild == NULL || node->rightChild == NULL){
        ...
        strcpy(code[node->data],keepCode);//调用strcpy函数拷贝字符串，其中code的下标用节点的字符(data)得到
        return;
    }
    keepCode[length] = '0';
...
void toVector(haffNode *node, int i)
{
    a[i][a[i][0]]=node->weight;
    a[i][0]++;
    if(node->leftChild == NULL || node->rightChild == NULL){
        return;
    }
    if(node->leftChild != NULL)
        toVector(node->leftChild,i+1);
    if(node->rightChild != NULL)
        toVector(node->rightChild,i+1);
}
void paintTree(haffNode *node)
{
...
void Zip::mainZip(QImage img){
    int count[256]={0};//用来统计字符个数，一开始清零，其中它的下标号表示这个字符的ASCII码
    char keepCode[256];//用于在生成编码的时候 临时储存编码 ，真正储存编码的地方看代码最上面的code[][]数组
    char reder;//用来存文件中的字符
    int fileLength=0;//用来计算原文长度
    int zipLength=0;//用来计算压缩文长度
    int i;
    int num=0;//用来计算出现的字符的个数 和其它一些计数功能
    haffNode node[256];//用来储存哈夫曼节点，这里我申请100个空间事实上不需要那么大，大概需要26+26+20（26个英文字母大小写+标点符号）
    FILE *fpw = fopen("D:\\output1","wb");//写入文件，wb是写入二进制文件，路径设置随意，但是要符合格式，写入的时候文件自动生成。
    FILE *fpr1= fopen("D:\\output1","rb");;//用于解压缩时读入文件 ，rb是读入二进制文件
    FILE *fpw1 = fopen("D:\\output3","w"); //用于解压缩时写入文件
    //解压需要用的
    char op;
    haffNode *z;
    //读取文件
    ...
    for(i=0; i<256; i++){
        if(count[i]!=0){
            node[num].data=i;//之前说过，下标就是出现的像素灰度值
    ...
    for(int i = 0; i< w; i++)
        for(int j = 0; j < h; j++)
            ...
            int b=qBlue(pixel);
            int a=(r*30+g*59+b*11)/100;
                for(int p=0; p<strlen(code[a]); p++){
                saveChar |= code[a][p]-'0';//让saveChar和编码中的每一位进行或操作，用字符的'1'-'0'，就可得到0000 0001.
                num++;
                if(num == 8){
